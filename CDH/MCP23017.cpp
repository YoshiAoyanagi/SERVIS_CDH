...
bool MCP23017::init(unsigned char addr)
{
	bool i_ver;

	begin(addr);

	i_ver = Wire1_beginTransmission(addr);
...
uint8_t MCP23017::readRegister(uint8_t addr) {
	...
	Wire1_beginTransmission(i2caddr);
	wiresend(addr);
	Wire1_endTransmission();
	Wire1_requestFrom(i2caddr, 1);
	return wirerecv();
}
