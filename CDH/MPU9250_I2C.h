...
class MPU9250_I2C{
  ...
  public:
	...
	SensData accelerometer;
	SensData gyroscope;
	SensData magnetometer;
	SensData temperature;

	int				Init(unsigned char low_pass_filter);

	void			beginTransmission(unsigned char address);
	unsigned char	endTransmission(void);
	int				write(unsigned char data);
	unsigned char	read(void);
	unsigned char	requestFrom(unsigned char address, unsigned char quantity);

	void			WriteReg(unsigned char address, unsigned char reg, unsigned char data);
	void			ReadRegs(unsigned char address, unsigned char reg, unsigned char *data, unsigned int len);
	unsigned int	Set_Accel_Range(int range);
	unsigned int	Set_Gyro_Range(int range);
	unsigned int	Calib_Magnetometer(void);
	unsigned int	WhoAmI(void);
	unsigned int	MAG_WhoAmI(void);
	...
	SensData		GenCalibrateVal(SensData sens);
	void			Set_GyroCalibParameters(float scale[3], float offset[3]);
	void			Set_AccelCalibParameters(float scale[3], float offset[3]);
	void			Set_MagCalibParameters(float scale[3], float offset[3]);
	int				Set_LPF(int LPF);
